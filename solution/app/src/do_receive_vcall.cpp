...
int run_receive_vcall(std::string const & device, serial_port_param_set_t const & param_set)
{
        ...
        return [&](boost::system::error_code const & ec, std::size_t rb)
            {
                ...
                else
                {
                    ...
                    bool ring_received = response.body.size() and response.body.front().rfind("RING") == 0;

                    if (ring_received)
                    {
                        fmt::print("Received RING, respond with ATA\n");
                        auto ata = Ringbeller::make_ata();

                        boost::system::error_code ec;
                        Ringbeller::write(sp, ata, ec);

                        fmt::print("You can capture PCM audio now\n");
                    }
    ...
    fmt::print("Waiting for the call. Press Ctrl-C to terminate\n");
    Ringbeller::async_read(sp, buf, response, make_call_read_handler());
    ios.run();
